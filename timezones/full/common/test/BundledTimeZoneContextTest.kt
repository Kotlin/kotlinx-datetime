...
class BundledTimeZoneContextTest {
    ...
    @Test
    fun utc() {
        assertEquals(TimeZone.UTC, BundledTimeZoneContext.get("UTC"))
        assertEquals(TimeZone.UTC, BundledTimeZoneContext.getOrNull("UTC"))
        assertIs<FixedOffsetTimeZone>(BundledTimeZoneContext.get("UTC"))
    }

    @Test
    fun system() {
        assertEquals(
            TimeZoneContext.System.currentTimeZoneId(), BundledTimeZoneContext.currentTimeZone().id
        )
        assertEquals(
            TimeZoneContext.System.currentTimeZoneId(), BundledTimeZoneContext.currentTimeZoneId()
        )
    }

    @Test
    fun available() {
        val allTzIds = BundledTimeZoneContext.availableZoneIds()
        ...
        assertContains(allTzIds, "UTC", "The UTC timezone not in $allTzIds")
    }
    ...
    fun availableZonesAreAvailable() {
        ...
        for (zoneName in BundledTimeZoneContext.availableZoneIds()) {
            val timezone = try {
                BundledTimeZoneContext.get(zoneName)
            } catch (e: Exception) {
    ...
    @Test
    fun of() {
        val tzm = BundledTimeZoneContext.get("Europe/Moscow")
        assertNotNull(tzm)
        assertEquals("Europe/Moscow", tzm.id)
        assertFailsWith<IllegalTimeZoneException> { BundledTimeZoneContext.get("Mars/Standard") }
        assertFailsWith<IllegalTimeZoneException> { BundledTimeZoneContext.get("UTC+X") }
        assertNull(BundledTimeZoneContext.getOrNull("Mars/Standard"))
        assertNull(BundledTimeZoneContext.getOrNull("UTC+X"))
    }

    // from 310bp
    @Test
    fun timeZoneEquals() {
        val test1 = BundledTimeZoneContext.get("Europe/London")
        val test2 = BundledTimeZoneContext.get("Europe/Paris")
        val test2b = BundledTimeZoneContext.get("Europe/Paris")
    ...
    fun timeZoneToString() {
        ...
        for ((id, str) in idToString) {
            assertEquals(str, BundledTimeZoneContext.get(id).toString())
        }
    }
    ...
    @Test
    fun newYorkOffset() {
        val test = BundledTimeZoneContext.get("America/New_York")
    ...
    fun checkKnownTimezoneDatabaseRecords() {
        with(BundledTimeZoneContext.get("America/New_York")) {
        ...
        with(BundledTimeZoneContext.get("Europe/Berlin")) {
        ...
        with(BundledTimeZoneContext.get("Europe/Moscow")) {
        ...
        with(BundledTimeZoneContext.get("Australia/Sydney")) {
