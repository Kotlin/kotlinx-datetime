...
public class UnresolvedZonedDateTime(
    ...
    public fun resolve(transitionHandler: TransitionHandler): Instant =
        rawLocalDateTime.toInstant(timeZone, transitionHandler, preferredOffset)
...
public fun UnresolvedZonedDateTime.plus(period: DateTimePeriod, transitionHandler: TransitionHandler): Instant = try {
    plus(period.datePeriodPortion())
        .resolve(transitionHandler).plus(period.totalNanoseconds, DateTimeUnit.NANOSECOND)
...
public fun UnresolvedZonedDateTime.minus(period: DateTimePeriod, transitionHandler: TransitionHandler): Instant =
    ...
    if (period.totalNanoseconds != Long.MIN_VALUE) {
        ...
        plus(negatedPeriod, transitionHandler)
    } else {
        val negatedPeriod = with(period) { buildDateTimePeriod(-totalMonths, -days, -(totalNanoseconds + 1)) }
        plus(negatedPeriod, transitionHandler).plus(1L, DateTimeUnit.NANOSECOND)
    }
...
// Depending on the 'transitionHandler' and on whether the 'other' is the moment before or after the DST change,
...
public fun UnresolvedZonedDateTime.until(
    ...
    val timeAfterAddingDate = withDate { otherLdt.date }.resolve(transitionHandler)
    val delta = when {
...
public fun Instant.minus(
    ...
    unit: DateTimeUnit.DateBased,
    transitionHandler: TransitionHandler,
): Long = other.until(this, unit, transitionHandler)

public fun UnresolvedZonedDateTime.daysUntil(other: Instant, transitionHandler: TransitionHandler): Int =
    until(other, DateTimeUnit.DAY, transitionHandler).clampToInt()

public fun UnresolvedZonedDateTime.monthsUntil(other: Instant, transitionHandler: TransitionHandler): Int =
    until(other, DateTimeUnit.MONTH, transitionHandler).clampToInt()

public fun UnresolvedZonedDateTime.yearsUntil(other: Instant, transitionHandler: TransitionHandler): Int =
    until(other, DateTimeUnit.YEAR, transitionHandler).clampToInt()

public fun UnresolvedZonedDateTime.periodUntil(other: Instant, transitionHandler: TransitionHandler): DateTimePeriod {
    val otherLdt = other.toLocalDateTime(timeZone)
    val timeAfterAddingDate = withDate { otherLdt.date }.resolve(transitionHandler)
    val delta = when {
        otherLdt.date > rawLocalDateTime.date && timeAfterAddingDate > other -> -1 // addition won't throw: end date - date >= 1
    ...
    val nanoseconds =
        withDate { endDate }.resolve(transitionHandler).until(other, DateTimeUnit.NANOSECOND) // |otherLdt - thisLdt| < 24h
...
public fun Instant.minus(other: UnresolvedZonedDateTime, transitionHandler: TransitionHandler): DateTimePeriod =
    other.periodUntil(this, transitionHandler)


/// COMBINED DISTANCE FUNCTIONS

public fun Instant.until(other: Instant, unit: DateTimeUnit, transitionHandler: TransitionHandler, timeZone: TimeZone): Long =
    when (unit) {
        is DateTimeUnit.DateBased -> unresolve(timeZone).until(other, unit, transitionHandler)
        is DateTimeUnit.TimeBased -> until(other, unit)
    }

public fun Instant.minus(other: Instant, unit: DateTimeUnit, transitionHandler: TransitionHandler, timeZone: TimeZone): Long =
    other.until(this, unit, transitionHandler, timeZone)
