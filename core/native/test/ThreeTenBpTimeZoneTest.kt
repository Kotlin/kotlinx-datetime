...
class ThreeTenBpTimeZoneTest {
    ...
    fun zoneOffsetToString() {
        var offset: FixedOffsetTimeZone = FixedOffsetTimeZone.ofHoursMinutesSeconds(1, 0, 0)
        assertEquals("+01:00", offset.toString())
        offset = FixedOffsetTimeZone.ofHoursMinutesSeconds(1, 2, 3)
        assertEquals("+01:02:03", offset.toString())
        offset = FixedOffsetTimeZone.UTC
    ...
    fun utcIsCached() {
        val values = arrayOf(
            ...
            "-00", "-0000", "-00:00", "-000000", "-00:00:00")
        for (v in values) {
            val test = FixedOffsetTimeZone.of(v)
            assertSame(test, FixedOffsetTimeZone.UTC)
        }
    }
    ...
    fun invalidZoneOffsetNames() {
        val values = arrayOf(
            ...
            "@01:00")
        for (v in values) {
            assertFailsWith(IllegalTimeZoneException::class, "should fail: $v") { FixedOffsetTimeZone.of(v) }
        }
    }

    private fun zoneOffsetCheck(offset: FixedOffsetTimeZone, hours: Int, minutes: Int, seconds: Int) {
        ...
        assertEquals(FixedOffsetTimeZone.ofHoursMinutesSeconds(hours, minutes, seconds), offset)
        assertEquals(offset, FixedOffsetTimeZone.of(id))
        assertEquals(id, offset.toString())
    }

    @Test
    fun zoneOffsetEquals() {
        val offset1 = FixedOffsetTimeZone.ofHoursMinutesSeconds(1, 2, 3)
        val offset2 = FixedOffsetTimeZone.ofHoursMinutesSeconds(2, 3, 4)
        val offset2b = FixedOffsetTimeZone.ofHoursMinutesSeconds(2, 3, 4)
            ...
            for (j in -59..59) {
                for (k in -59..59) {
                    if (i < 0 && j <= 0 && k <= 0 || i > 0 && j >= 0 && k >= 0 ||
                        ...
                        val test = FixedOffsetTimeZone.of(str)
                        zoneOffsetCheck(test, i, j, k)
                    }
                }
            }
        }
        val test1 = FixedOffsetTimeZone.of("-18:00:00")
        zoneOffsetCheck(test1, -18, 0, 0)
        val test2 = FixedOffsetTimeZone.of("+18:00:00")
    ...
    fun overlappingLocalTime() {
        ...
        assertEquals(ZonedDateTime(LocalDateTime(2007, 10, 28, 2, 30, 0, 0),
            zone, FixedOffsetTimeZone.ofSeconds(2 * 3600).offset), with(zone) { t.atZone() })
    }
