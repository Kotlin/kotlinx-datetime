...
internal fun timeZoneByIdFoundationOrNull(zoneId: String): TimeZone? = NSTimeZone.timeZoneWithName(zoneId)?.let {
    RuleBasedTimeZoneCalculations(TimeZoneRulesFoundation(it), zoneId, it).asTimeZone()
}
...
private val tzdb = tryInitializeTimezoneDatabase {
    TzdbOnFilesystem(Path.fromString(defaultTzdbPath()))
}
...
internal actual val timeZoneDatabaseImpl: TimeZoneDatabase = object: TimeZoneDatabase {
    ...
    override fun getOrNull(id: String): TimeZone? =
        if (tzdb !is UninitializedTimeZoneDatabase)
            ...
            timeZoneByIdFoundationOrNull(id)

    override fun availableZoneIds(): Set<String> = if (tzdb !is UninitializedTimeZoneDatabase)
...
internal actual fun currentSystemDefaultTimeZone(): TimeZone =
    TimeZoneContext.System.get(TimeZoneContext.System.currentTimeZoneId())
