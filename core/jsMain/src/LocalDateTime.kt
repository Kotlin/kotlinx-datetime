...
actual operator fun LocalDateTime.minus(other: LocalDateTime): CalendarPeriod {
    val timeNanoDiff = this.value.toLocalTime().toNanoOfDay().toDouble() - other.value.toLocalTime().toNanoOfDay().toDouble()
    ...
    val resultPeriod = jtPeriod.between(other.value.toLocalDate(), this.value.toLocalDate().plusDays(borrowDay))

    return with(resultPeriod) {
        resultTime.toComponents { hours, minutes, seconds, nanoseconds ->
            CalendarPeriod(years = years().toInt(), months = months().toInt(), days = days().toInt(), hours = hours, minutes = minutes, seconds = seconds.toLong(), nanoseconds = nanoseconds.toLong())
        }
    }
}